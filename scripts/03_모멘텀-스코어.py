...
-------------------------------
사용법:
  python 03_모멘텀_스코어.py --date 20260630 [--output <png경로>] [--month 6]
...
OUTPUT = args.output or f'03_모멘텀_스코어_{args.month}월.png'
BASE_DATE = args.date
...
d12m = (datetime.strptime(base,'%Y%m%d') - timedelta(days=365)).strftime('%Y%m%d')
...
for ticker, name in ETF_MAP.items():
    try:
        ...
        p_12m  = get_price(ticker, d12m)

        if None in [p_base, p_1m, p_3m, p_6m, p_12m]:
        ...
        r12m = (p_base - p_12m) / p_12m * 100
        score = r1m + r3m + r6m + r12m

        results.append({'ticker': ticker, 'name': name,
                        '1M': round(r1m,2), '3M': round(r3m,2),
                        '6M': round(r6m,2), '12M': round(r12m,2),
                        'score': round(score,2)})
        print(f"  {name}: 1M={r1m:.1f}% 3M={r3m:.1f}% 6M={r6m:.1f}% 12M={r12m:.1f}% → {score:.1f}")
...
for i, row in res_df.iterrows():
    ...
    detail = f"1M {row['1M']:+.1f}%   3M {row['3M']:+.1f}%   6M {row['6M']:+.1f}%   12M {row['12M']:+.1f}%"
...
ax.set_xlabel('모멘텀 스코어 (1M+3M+6M+12M 수익률 합산, %)', fontsize=9.5, color=GRAY_TEXT, labelpad=8)
...
fig.text(0.04, 0.91, f'기준일: {BASE_DATE[:4]}.{BASE_DATE[4:6]}.{BASE_DATE[6:]}  ·  스코어 = 1M+3M+6M+12M 수익률 합산  ·  좌측 = 순위',
         fontsize=10, color=GRAY_TEXT, va='top')
fig.add_artist(plt.Line2D([0.04,0.5],[0.905,0.905], transform=fig.transFigure, color=GREEN, linewidth=1.5, alpha=0.7))
fig.add_artist(plt.Line2D([0.5,0.96],[0.905,0.905], transform=fig.transFigure, color=GRAY_LINE, linewidth=1.5))
...
ax.legend(handles=[green_patch, red_patch], loc='lower right', fontsize=9, frameon=False,
          labelcolor=GRAY_TEXT, bbox_to_anchor=(0.58, -0.08))
